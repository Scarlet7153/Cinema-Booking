...
const DateSelect = ({ dateTime, id }) => {
    ...
    const onBookHandler = () => {
      if (!selectedDate) return toast('Vui lòng chọn ngày');
  ...
  return (
    <div id="dateSelect" className="pt-30">
      <div className="flex flex-col md:flex-row items-center justify-between gap-10 relative p-8 bg-primary/10 border-primary/20 rounded-lg">
            ...
            <span className="grid grid-cols-3 md:flex flex-wrap md:max-w-lg gap-4">
              {Object.keys(dateTime).map((date) => (
                ...
                >
                  ...
                  <span>
                    {new Date(date).toLocaleString("vi-VN", { month: "short" })}
                  </span>
                </button>
              ))}
            </span>
        ...
        <button onClick={onBookHandler} className="bg-primary text-white px-8 mt-6 py-2 rounded hover:bg-primary/90 transition-all cursor-pointer">
          Đặt ngay
        </button>
      </div>
    </div>
